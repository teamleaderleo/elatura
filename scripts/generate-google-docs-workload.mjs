...
function usage() {
  console.error("Usage: node scripts/generate-google-docs-workload.mjs --out <directory>");
}
...
function fixedParagraph(fixtureId, documentOrdinal, paragraphOrdinal, anchorEvery) {
  const anchorOrdinal =
...
function buildDocument(fixture, documentOrdinal) {
  const paragraphs = [];
  for (let paragraphOrdinal = 0; paragraphOrdinal < fixture.paragraphs; paragraphOrdinal += 1) {
    paragraphs.push(
      fixedParagraph(
        fixture.id,
        documentOrdinal,
