...
function verifyBackground(background) {
  ...
  assert.equal(
    background.includes("manualBrowserDiscardEligibility(current)"),
    true,
    "Manual discard must revalidate fresh browser state",
  );
  assert.equal(background.includes("lane-governor"), false, "Projection host must not embed a second resource governor");
}

function verifyProjectionBoundary(lifecycle) {
  assert.equal(lifecycle.includes("projectionIdForTab"), true, "Projection token must be explicit");
  assert.equal(lifecycle.includes("laneIdForTab"), false, "Browser tab ids must not be labeled as lane identity");
  assert.equal(lifecycle.includes("laneRef"), false, "Durable lane references stay outside browser projection code");
}
...
async function main() {
  ...
  verifyProjectionBoundary(lifecycle);
